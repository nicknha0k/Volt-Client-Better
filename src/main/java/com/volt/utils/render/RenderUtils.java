...
public final class RenderUtils {
    ...
    public static void unscaledProjection() {
        ...
        RenderSystem.setProjectionMatrix(new Matrix4f().setOrtho(0, Volt.mc.getWindow().getFramebufferWidth(), Volt.mc.getWindow().getFramebufferHeight(), 0, 1000, 21000), VertexSorter.BY_Z);
        rendering3D = false;
    }

    public static void scaledProjection() {
        RenderSystem.setProjectionMatrix(new Matrix4f().setOrtho(0, (float) (Volt.mc.getWindow().getFramebufferWidth() / Volt.mc.getWindow().getScaleFactor()), (float) (Volt.mc.getWindow().getFramebufferHeight() / Volt.mc.getWindow().getScaleFactor()), 0, 1000, 21000), vertexSorter);
        rendering3D = true;
    }
    ...
    public static void drawRoundedRect(DrawContext context, int x, int y, int width, int height, int radius, int color) {
        drawRoundedRectWithCorners(context, x, y, width, height, radius, true, true, true, true, color);
    }

    /**
     * Retângulo arredondado com cantos independentes (tl = topo-esq, tr = topo-dir,
     * bl = baixo-esq, br = baixo-dir). Mesmo custo do arredondado cheio quando
     * todos ligados; barato e batch-friendly (usa fills do DrawContext).
     * As faixas NUNCA se sobrepõem: cada pixel é desenhado uma única vez,
     * então cores translúcidas não criam listras de tom diferente.
     */
    public static void drawRoundedRectWithCorners(DrawContext context, int x, int y, int width, int height, int radius,
        ...
        // Faixa superior (só entre os cantos)
        context.fill(x + r, y, x + width - r, y + r, color);
        // Faixa do meio (largura total)
        context.fill(x, y + r, x + width, y + height - r, color);
        // Faixa inferior (só entre os cantos)
        context.fill(x + r, y + height - r, x + width - r, y + height, color);

        if (tl) drawRoundedCorner(context, x + r, y + r, r, color, 0);
        else context.fill(x, y, x + r, y + r, color);
        if (tr) drawRoundedCorner(context, x + width - r, y + r, r, color, 1);
        else context.fill(x + width - r, y, x + width, y + r, color);
        if (bl) drawRoundedCorner(context, x + r, y + height - r, r, color, 2);
        else context.fill(x, y + height - r, x + r, y + height, color);
        if (br) drawRoundedCorner(context, x + width - r, y + height - r, r, color, 3);
        else context.fill(x + width - r, y + height - r, x + width, y + height, color);
    }

    /** Borda arredondada de 1px (barata: só o anel). */
    public static void drawRoundedBorder(DrawContext context, int x, int y, int width, int height, int radius, int color) {
        ...
        drawRoundedBorderCorner(context, x + r, y + r, r, color, 0);
        drawRoundedBorderCorner(context, x + width - r, y + r, r, color, 1);
        drawRoundedBorderCorner(context, x + r, y + height - r, r, color, 2);
        drawRoundedBorderCorner(context, x + width - r, y + height - r, r, color, 3);
    }

    private static void drawRoundedBorderCorner(DrawContext context, int centerX, int centerY, int radius, int color, int corner) {
        for (int i = 0; i < radius; i++) {
            for (int j = 0; j < radius; j++) {
                float distance = (float) Math.sqrt(i * i + j * j);
                if (distance >= radius - 1.2f && distance <= radius) {
                    int pixelX = centerX;
                    int pixelY = centerY;
                    switch (corner) {
                        case 0 -> {
