...
public class SettingsRenderer {
    ...
    public static int renderModuleSettings(DrawContext context, Module module, int x, int moduleY, int width, float animation, FontRenderer smallFont, Map<ModeSetting, Boolean> dropdownExpanded, GuiEventHandler eventHandler) {
        ...
        RenderUtils.drawRoundedRect(context, x + 18, moduleY, width - 36, animatedHeight, 6, new Color(15, 15, 25, 200).getRGB());
    ...
    private static void renderModeDropdownOverlay(DrawContext context, ModeSetting modeSetting, int x, int y, int width, int height) {
        ...
        RenderUtils.drawRoundedRect(context, x, y, width, height, 3, new Color(70, 70, 90).getRGB());
        RenderUtils.drawRoundedBorder(context, x, y, width, height, 3, new Color(100, 100, 100).getRGB());
        ...
        for (String mode : modeSetting.getModes()) {
            ...
            RenderUtils.drawRoundedRect(context, x, optionY, width, height, 3, bg.getRGB());
    ...
    public static int renderModuleSettingsInGrid(DrawContext context, Module module, int x, int moduleY, int width, float animation, FontRenderer smallFont) {
        ...
        RenderUtils.drawRoundedRect(context, x + 18, moduleY, width - 36, animatedHeight, 6,
    ...
    private static void renderSettingControlCommon(DrawContext context, Setting setting, int controlX, int controlY, int controlWidth, int controlHeight, Map<ModeSetting, Boolean> dropdownExpanded, GuiEventHandler eventHandler) {
        ...
        switch (setting) {
            ...
            case ModeSetting modeSetting -> {
                ...
                RenderUtils.drawRoundedRect(context, controlX, controlY, controlWidth, controlHeight, 4,
                        new Color(70, 70, 90).getRGB());
                RenderUtils.drawRoundedBorder(context, controlX, controlY, controlWidth, controlHeight, 4,
                ...
                if (isExpanded) {
                    ...
                    for (String mode : modeSetting.getModes()) {
                        ...
                        RenderUtils.drawRoundedRect(context, controlX, optionY, controlWidth, controlHeight, 3,
                                optionBg.getRGB());
            ...
            case KeybindSetting keybindSetting -> {
                ...
                RenderUtils.drawRoundedRect(context, controlX, controlY, controlWidth, controlHeight, 4, bgColor.getRGB());
                RenderUtils.drawRoundedBorder(context, controlX, controlY, controlWidth, controlHeight, 4,
                        new Color(100, 100, 100).getRGB());
            ...
            case StringSetting stringSetting -> {
                boolean listening = eventHandler != null && stringSetting.equals(eventHandler.getListeningString());
                Color bgColor = listening ? new Color(60, 50, 90) : new Color(40, 40, 50);
                Color borderColor = listening ? new Color(150, 100, 255) : new Color(100, 100, 100);

                RenderUtils.drawRoundedRect(context, controlX, controlY, controlWidth, controlHeight, 4, bgColor.getRGB());
                RenderUtils.drawRoundedBorder(context, controlX, controlY, controlWidth, controlHeight, 4, borderColor.getRGB());

                String value = stringSetting.getValue() == null ? "" : stringSetting.getValue();
                var textRenderer = MinecraftClient.getInstance().textRenderer;
                int maxTextWidth = controlWidth - 8;
                String display = value;
                if (display.isEmpty() && !listening) display = "...";
                while (textRenderer.getWidth(display) > maxTextWidth && display.length() > 1) {
                ...
                if (listening && (System.currentTimeMillis() % 1000) < 500) {
                    int cursorX = controlX + 4 + textRenderer.getWidth(display);
                    if (cursorX < controlX + controlWidth - 2) {
                        context.fill(cursorX, controlY + 2, cursorX + 1, controlY + controlHeight - 2, Color.WHITE.getRGB());
                    }
                }
            }
