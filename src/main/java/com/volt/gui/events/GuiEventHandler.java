...
public class GuiEventHandler {
    ...
    public boolean handleMouseClick(double mouseX, double mouseY, int button, int screenWidth, int screenHeight, List<Module> modules) {
        ...
        // Clicou no vazio: tira o foco do campo de texto
        listeningString = null;
        return false;
    }
    ...
    private boolean handleSpecificSettingClick(Setting setting, double mouseX, double mouseY, int controlX, int controlY, int controlWidth) {
        if (!(setting instanceof StringSetting)) listeningString = null;
        switch (setting) {
    ...
    public boolean handleKeyPress(int keyCode, int scanCode, int modifiers) {
        if (listeningString != null) {
            ...
            if (keyCode == GLFW.GLFW_KEY_BACKSPACE) {
                String v = listeningString.getValue();
                if (v != null && !v.isEmpty()) {
                    listeningString.setValue(v.substring(0, v.length() - 1));
                }
                return true;
            }
            // Delete limpa tudo
            if (keyCode == GLFW.GLFW_KEY_DELETE) {
                listeningString.setValue("");
                return true;
            }
            return true;
        }
    ...
    public boolean handleCharTyped(char chr, int modifiers) {
        if (listeningString != null) {
            // Nicks de MC: letras, números e underscore, até 16 chars. Textos gerais: até 24.
            boolean isNameBox = "Skin Name".equals(listeningString.getName());
            String v = listeningString.getValue() == null ? "" : listeningString.getValue();
            int maxLen = isNameBox ? 16 : 24;
            if (v.length() < maxLen) {
                if (isNameBox) {
                    if (Character.isLetterOrDigit(chr) || chr == '_') {
                        listeningString.setValue(v + chr);
                    }
    ...
    public StringSetting getListeningString() {
        return listeningString;
    }

    public void setListeningString(StringSetting listeningString) {
        this.listeningString = listeningString;
    }
