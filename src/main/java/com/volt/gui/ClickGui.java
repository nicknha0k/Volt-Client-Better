...
public final class ClickGui extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        RenderUtils.drawRoundedRect(context, containerX, containerY, containerWidth, containerHeight, 12,
    ...
    private void renderHeader(DrawContext context, int x, int y, int width, int mouseX, int mouseY) {
        ...
        RenderUtils.drawRoundedRectWithCorners(context, x, y, width, HEADER_HEIGHT, 12,
            true, true, false, false, new Color(35, 35, 50, headerAlpha).getRGB());
    ...
    private void renderSearchBar(DrawContext context, int x, int y, int width) {
        ...
        int pillRadius = SEARCH_BAR_HEIGHT / 2;
        boolean searchActive = eventHandler.isSearchFocused() || !eventHandler.getSearchQuery().isEmpty();
        Color pillBorder = searchActive ? new Color(150, 100, 255, 220) : new Color(70, 70, 90, 200);
        RenderUtils.drawRoundedRect(context, searchX, searchY, SEARCH_BAR_WIDTH, SEARCH_BAR_HEIGHT, pillRadius,
            new Color(40, 40, 50, 200).getRGB());
        RenderUtils.drawRoundedBorder(context, searchX, searchY, SEARCH_BAR_WIDTH, SEARCH_BAR_HEIGHT, pillRadius,
            pillBorder.getRGB());
    ...
    private void renderSidebar(DrawContext context, int x, int y, int height, int mouseX, int mouseY) {
        ...
        for (Category category : Category.values()) {
            ...
            if (newAnimation > 0.02f) {
                int pillAlpha = (int)(newAnimation * (isSelected ? 90 : 60));
                RenderUtils.drawRoundedRect(context, sidebarX + 10, categoryY, SIDEBAR_WIDTH - 20, 35, 9,
                    new Color(150, 100, 255, Math.min(255, pillAlpha)).getRGB());
            }
    ...
    private void renderModuleContent(DrawContext context, int x, int y, int width, int height, int mouseX, int mouseY) {
        ...
        for (Module module : modules) {
			...
			RenderUtils.drawRoundedRect(context, x + PADDING, moduleY, width - PADDING * 2, MODULE_HEIGHT, 7, bgColor.getRGB());
			int indicatorAlpha = isEnabled ? (int)(newAnimation * 255) : 0;
			if (indicatorAlpha > 0) {
				Color indicator = new Color(150, 100, 255, indicatorAlpha);
				RenderUtils.drawRoundedRect(context, x + PADDING + 3, moduleY + 6, 3, MODULE_HEIGHT - 12, 2, indicator.getRGB());
			}
    ...
    private void renderConfigContent(DrawContext context, int x, int y, int width, int height, int mouseX, int mouseY) {
        ...
        RenderUtils.drawRoundedRect(context, x + PADDING, currentY, width - PADDING * 2, 30, 7, new Color(40, 40, 50, 200).getRGB());
        RenderUtils.drawRoundedBorder(context, x + PADDING, currentY, width - PADDING * 2, 30, 7,
            (configNameFocused ? new Color(150, 100, 255, 220) : new Color(70, 70, 90, 200)).getRGB());
        ...
        for (String config : configs) {
            ...
            RenderUtils.drawRoundedRect(context, x + PADDING, currentY, width - PADDING * 2, 25, 6, bgColor.getRGB());
    ...
    private void renderConfigButton(DrawContext context, int x, int y, int width, int height, String text, Color baseColor, int mouseX, int mouseY) {
        ...
        Color buttonColor = isHovered ? new Color(Math.min(255, baseColor.getRed() + 20), Math.min(255, baseColor.getGreen() + 20), Math.min(255, baseColor.getBlue() + 20)) : baseColor;
        RenderUtils.drawRoundedRect(context, x, y, width, height, 6, buttonColor.getRGB());
